...
var _ArrayPrototypeValues = require("array.prototype.values");
var _ArrayPrototypeConcat = require("array.prototype.concat");
...
function old_convertMetadataMapToFinal(obj, metadataMap) {
  ...
  if (0 !== metadataKeys.length) {
    for (var i = 0; i < metadataKeys.length; i++) {
      ...
      if (priv) {
        var _priv, _privArr;
        var privArr = _ArrayFrom((_priv = priv, Array.isArray(_priv) ? _ArrayPrototypeValues : Function.call.bind(_priv.values))(_priv)),
          parentPriv = parentMetaForKey ? parentMetaForKey["private"] : null;
        parentPriv && (privArr = (_privArr = privArr, Array.isArray(_privArr) ? _ArrayPrototypeConcat : Function.call.bind(_privArr.concat))(_privArr, parentPriv)), metaForKey["private"] = privArr;
      }
...
function old_memberDec(dec, name, desc, metadataMap, initializers, kind, isStatic, isPrivate, value) {
  ...
  try {
    return dec(value, _ObjectAssign(ctx, old_createMetadataMethodsForProperty(metadataMap, metadataKind, metadataName, decoratorFinishedRef)));
  } finally {
...
function old_applyClassDecs(ret, targetClass, metadataMap, classDecs) {
  if (classDecs.length > 0) {
    for (var initializers = [], newClass = targetClass, name = (_targetClass = targetClass, typeof _targetClass === "function" ? _FunctionPrototypeName(_targetClass) : _targetClass.name), i = classDecs.length - 1; i >= 0; i--) {
      var _targetClass;
      var decoratorFinishedRef = {
        v: !1
      };
      try {
        var ctx = _ObjectAssign({
            kind: "class",
            name: name,
